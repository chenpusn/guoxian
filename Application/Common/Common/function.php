 ...
 * @param array $credit
 *        	自定义金币值，格式：array('score'=>金币值,'experience'=>经历值);为空时默认取管理中心积分管理里的配置值
 * @param int $admin_uid
 ...
 */
function get_lottery1($prizeArr, $start_time, $end_time, $event_id = 0, $uid = 0, $update = false) {
	if (empty ( $uid )) {
		$uid = get_mid ();
	}
...
function del_lottery1($index, $event_id = 0, $uid) {
	if (empty ( $uid )) {
		$uid = get_mid ();
	}
	$key = 'function_lottery_' . $uid . '_' . $event_id;
...
/**
 * 引用wechat-php-sdk类库，获取微信操作对象
 */
function getWechatApiObj(){

	$mpInfo = get_token_appinfo();

	$options = array(
    	'token'=>$mpInfo['interface_token'], 		//填写你设定的key
    	'encodingaeskey'=>$mpInfo['encodingaeskey'], 		//填写加密用的EncodingAESKey
    	'appid'=>$mpInfo['appid'], 				//填写高级调用功能的app id
...
function getRevId(){
  $weObj = getWechatApiObj();
  return $weObj->getRevID();
}

/*  
 * 返回消息发送时间
 */
function getRevCtime(){
  $weObj = getWechatApiObj();
  return $weObj->getRevCtime();
}
...
/*  
 * 返回图片信息（图片型信息） 返回数组{‘mediaid’=>’’,’picurl’=>’’}
 */
function getRevPic(){
  $weObj = getWechatApiObj();
  return $weObj->getRevPic();
}

/*
 * 接收消息链接（链接型信息） 返回数组{‘url’=>’’,’title’=>’’,’description’=>’’}
 */
function getRevLink(){
  $weObj = getWechatApiObj();
  return $weObj->getRevLink();
}

/*
 * 返回地理位置（位置型信息） 返回数组{‘x’=>’’,’y’=>’’,’scale’=>’’,’label’=>’’}
 */
function getRevGeo(){
  $weObj = getWechatApiObj();
  return $weObj->getRevGeo();
}

/*
 * 返回事件类型（事件型信息） 返回数组{'event'=>'','key'=>''}
 */ 
function getRevEvent(){
  $weObj = getWechatApiObj();
  return $weObj->getRevEvent();
}


/*
 * 返回语音信息（语音型信息） 返回数组{'mediaid'=>'','format'=>''}
 */
function getRevVoice(){
  $weObj = getWechatApiObj();
  return $weObj->getRevVoice();
}

/*
 * 返回视频信息（视频型信息） 返回数组{'mediaid'=>'','thumbmediaid'=>''}
 */
function getRevVideo(){
  $weObj = getWechatApiObj();
  return $weObj->getRevVideo();
}
...
function sendSMS($mobile, $content){
	...
	$post_data['sign']=strtolower(md5('a1042854654213'.(date("YmdHis",$time))));//MD5（账号密码当前时间戳）
