...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?>
            <?php if($add_button || $del_button || $search_button || !empty($top_more_button) || !empty($muti_search)): ?><div class="table-bar">
                    <div class="fl">
                        <?php if(empty($model["extend"])): ?><div class="tools">
                                ...
                                <?php if($del_button): $del_url || $del_url = U('del?model='.$model['id'], $get_param); ?>
                                    <button class="btn ajax-post confirm" target-form="ids" url="<?php echo ($del_url); ?>">删 除
                                    </button><?php endif; ?>
                                <?php if(is_array($top_more_button)): $i = 0; $__LIST__ = $top_more_button;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo[is_buttion]): ?><button class="btn <?php echo ($vo["class"]); ?>" target-form="ids" url="<?php echo ($vo["url"]); ?>"><?php echo ($vo["title"]); ?>
                                        </button>
                                        <?php else: ?>
                    ...
                    <?php if($search_button): ?><div class="search-form fr cf">
                            <div class="sleft">
                                <input type="text" name="<?php echo ($search_key); ?>" class="search-input" value="<?php echo I($search_key);?>"
                                       placeholder="<?php echo ($placeholder); ?>">
                                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo ($search_url); ?>"><i
                            ...
                            <div class="" style="line-height: 30px;">
                                <?php if(is_array($muti_search)): $i = 0; $__LIST__ = $muti_search;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; switch($vo["type"]): case "select": ?><span><?php echo ($vo["title"]); ?>：</span>
                                            <select name="<?php echo ($vo["name"]); ?>" class="search-input input-small">
                                                <?php if(is_array($vo["options"])): $i = 0; $__LIST__ = $vo["options"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($i % 2 );++$i;?><option value="<?php echo ($option["value"]); ?>">
                                                        <?php if(($option["default_value"]) == "option.value"): ?>selected<?php endif; ?>
                                                        ><?php echo ($option["title"]); ?>
                                                    </option><?php endforeach; endif; else: echo "" ;endif; ?>
                                            </select><?php break;?>
                                        <?php case "datetime": ?><span><?php echo ($vo["title"]); ?>：</span>
                                            <input type="datetime" name="start_time"
                                                   class="search-input date input-small" value="<?php echo ($vo["start_time"]); ?>"
                                                   placeholder="请选择时间">
                                            <span>至</span>
                                            <input type="datetime" name="end_time" class="search-input date input-small"
                                                   value="<?php echo ($vo["end_time"]); ?>" placeholder="请选择时间"><?php break;?>
                                        <?php case "checkbox": ?><span><?php echo ($vo["title"]); ?>：</span>
                                            <?php if(is_array($vo["options"])): $i = 0; $__LIST__ = $vo["options"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($i % 2 );++$i;?><input type="checkbox" name="<?php echo ($option["name"]); ?>" class=""
                                                       value="<?php echo ($option["value"]); ?>">
                                                <?php if(($option["default_value"]) == "option.value"): ?>checked<?php endif; ?>
                                                ><?php echo ($option["title"]); endforeach; endif; else: echo "" ;endif; break;?>
                                        <?php case "radio": ?><span><?php echo ($vo["title"]); ?>：</span>
                                            <?php if(is_array($vo["options"])): $i = 0; $__LIST__ = $vo["options"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($i % 2 );++$i;?><input type="radio" name="<?php echo ($option["name"]); ?>" class=""
                                                       value="<?php echo ($option["value"]); ?>">
                                                <?php if(($option["default_value"]) == "option.value"): ?>checked<?php endif; ?>
                                                ><?php echo ($option["title"]); endforeach; endif; else: echo "" ;endif; break; endswitch; endforeach; endif; else: echo "" ;endif; ?>

                                <!-- <a class="sort " href="#" title="排序">排序:高->低</a> -->
                                <button type="submit" class="sch-btn btn" href="javascript:;" id="search" url="<?php echo ($search_url); ?>">搜索</button>
                            </div>
                        </form><?php endif; ?>
                </div><?php endif; ?>
            <!-- 数据列表 -->
            <div class="data-table">
                <div class="table-striped">
                    <table cellspacing="1">
                        ...
                        <thead>
                        <tr>
                            <?php if($check_all): ?><th class="row-selected row-selected"><input type="checkbox" id="checkAll"
                                                                             class="check-all regular-checkbox"><label
                                        for="checkAll"></label></th><?php endif; ?>
                            <?php if(is_array($list_grids)): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><!--<?php if(empty($field["width"])): ?><th><?php echo ($field["title"]); ?></th>
                                <?php else: ?>
                                  <th width="<?php echo ($field["width"]); ?>%"><?php echo ($field["title"]); ?></th><?php endif; ?>-->
                                <th><?php echo ($field["title"]); ?></th><?php endforeach; endif; else: echo "" ;endif; ?>
                        </tr>
                        </thead>
                        ...
                        <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                                <?php if($check_all): ?><td><input class="ids regular-checkbox" type="checkbox" value="<?php echo ($data['id']); ?>"
                                               name="ids[]" id="check_<?php echo ($data['id']); ?>"><label
                                            for="check_<?php echo ($data['id']); ?>"></label></td><?php endif; ?>
